...
class Conversation {
  static async findOrCreate(contact_wa_id, phone_number_id) {
    ...
    let { rows } = await pool.query(
      `SELECT * FROM conversations 
       WHERE contact_wa_id = $1 AND phone_number_id = $2 AND is_active = true`,
      [contact_wa_id, phone_number_id]
    );
    if (rows.length > 0) {
      return rows[0];
    }
    ...
    const insert = await pool.query(
      `INSERT INTO conversations (contact_wa_id, phone_number_id, is_active)
  ...
  static async getById(id) {
    const { rows } = await pool.query(
      `SELECT c.*, co.profile_name FROM conversations c
       JOIN contacts co ON co.wa_id = c.contact_wa_id
       WHERE c.id = $1`,
      [id]
    );
