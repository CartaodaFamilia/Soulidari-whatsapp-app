...
import { Send, Paperclip, MoreVertical, ChevronLeft, Phone, Video, Check, CheckCheck, Clock } from 'lucide-react';
...
const MessageBubble = ({ message }) => {
  ...
  return (
    <div className={`flex ${isOutbound ? 'justify-end' : 'justify-start'} mb-4`}>
      <div className={`max-w-[70%] px-4 py-2 rounded-2xl shadow-sm ${isOutbound ? 'bg-green-600 text-white rounded-tr-none' : 'bg-white text-gray-800 border border-gray-100 rounded-tl-none'}`}>
        <p className="text-sm leading-relaxed">{message.body}</p>
        <div className={`flex items-center justify-end gap-1 mt-1 ${isOutbound ? 'text-green-100' : 'text-gray-400'}`}>
          <span className="text-[10px]">{new Date(message.timestamp * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
          <StatusIcon />
        </div>
      </div>
    </div>
...
function ConversationView({ id: propId, embedded = false }) {
  ...
  useEffect(() => { loadData(); const i = setInterval(loadData, 5000); return () => clearInterval(i); }, [id]);
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between bg-white flex-shrink-0">
        <div className="flex items-center gap-4">
          {!embedded && (
            <button onClick={() => navigate(-1)} className="p-2 hover:bg-gray-50 rounded-full text-gray-500">
              <ChevronLeft size={20} />
            </button>
          )}
      ...
      <div className="p-4 bg-white border-t border-gray-50 flex-shrink-0">
        <form onSubmit={handleSend} className="flex items-center gap-3">
          ...
          <input
            ...
            className="flex-1 bg-gray-50 border-none focus:ring-2 focus:ring-green-500 rounded-xl px-4 py-3 text-sm"
          />
          <button type="submit" disabled={!inputText.trim() || sending}
            className={`p-3 rounded-xl shadow-lg transition-all ${!inputText.trim() || sending ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-green-600 text-white hover:bg-green-700'}`}>
            <Send size={20} />
          </button>
        </form>
      </div>
